...
**Via Neon Console**:
1. Go to SQL Editor
2. Manually rollback tables with DROP/CREATE
...
- **Neon Free**: 3GB storage, 20GB transfer, adequate for dev/staging
- **Render Free**: 750 hours/month, auto-sleep after 15 min inactivity
- **Render Pro**: $7/month, no auto-sleep, better for production

### When to Upgrade
- If database > 500MB, upgrade Neon
- If API response time increases, upgrade Render instance size
- If handling > 1000 concurrent users, consider CDN
...
- [ ] **Neon**: Create project, initialize schema, get connection string
- [ ] **Firebase**: Create project, download service account key, configure for Flutter
- [ ] **Render**: Connect GitHub, set up service, add environment variables
- [ ] **Secrets**: Generate JWT_SECRET, upload Firebase credentials
- [ ] **Database**: Verify connection, seed initial data
- [ ] **Flutter**: Update API endpoint, add Firebase configs, build APK/iOS
- [ ] **Testing**: Run health checks, test authentication, verify notifications
- [ ] **Monitoring**: Set up alerts, monitor logs, check performance
